...
def model(x,t,deltadot,v,l,a,b,delta):
#		delta = x[2]
		th1 = x[3]
		th2 = x[4]
...
class ODESolver:
	...
	def solve_ode_step(self,t,delta,v):
		...
		x = odeint(model,self.x0,tspan,args=(deltadot,v,self.l,self.a,self.b,delta))
		self.x0 = x[1]
		self.t_d1 = t
		self.delta_d1 = delta
		print self.x0
...
#		self.vel_sub = rospy.Subscriber("cmd_vel",Twist,self.vel_callback,queue_size=1)
	...
	def vel_callback(self,msg):
		...
		self.publish_articulation_angle(v,delta)

	def odom_callback(self,msg):
		[v,delta] = self.twist_to_ackerman(msg.twist.twist)
		self.publish_articulation_angle(v,delta)

	def publish_articulation_angle(self,v,delta):
		#if abs(v) > 0.01:
		t = rospy.get_time()
...
if __name__ == "__main__":
	...
	estimator = ArticulationAngleEstimator(0.99,0.99/2,(15+28)*0.0254)
	rospy.spin()
